...
use super::{
  defines::{parse_num, parse_token, parse_token_ok, ParserError},
...
impl ParseLine<Mtl> for MtlParserImpl {
  fn parse_line(
    ...
    let (my_current, texutures) = data.0.get_mutates();
    ...
    match my_current {
      Some(current) => {
        ...
        match s {
          ...
          "Ke" => {
            current.emissive_coeficient = parse_token_ok!(tokens.next();Vec3=x:f32,y:f32,z:f32)
          }
          "Tf" => {
            current.transmission_filter = parse_token_ok!(tokens.next();Vec3=x:f32,y:f32,z:f32)
          }
          ...
          "map_Ks" => {
            map.specular_color = parse_texture_token!(tokens.next();texutures;working_dir)
          }
          "map_Ns" => {
